...
kb = ReplyKeyboardMarkup(resize_keyboard=True, input_field_placeholder='Выберите кнопку')  # изменяет размер кнопок
button1 = KeyboardButton(text="Информация")  # создаёт кнопку с текстом
button2 = KeyboardButton(text="Рассчитать")
kb.add(button1, button2)  # добавляет кнопку на экран
...
async def on_start(message: types.Message):
    await message.answer('Привет!', reply_markup=kb)  # reply_markup добавляет кнопки после сообщения
...
@dp.message_handler(text=['Рассчитать'])  # хендлер реагирует на сообщение start
